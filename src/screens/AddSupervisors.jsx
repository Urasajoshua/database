...
const SupervisorForm = () => {
    ...
    useEffect(() => {
        const fetchSupervisors = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/users/?role=SUPERVISOR');
        ...
        const fetchStudents = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/users/?role=STUDENT');
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.post('http://127.0.0.1:8000/api/assign_students_to_supervisor/', formData).then((resonse)=>{
